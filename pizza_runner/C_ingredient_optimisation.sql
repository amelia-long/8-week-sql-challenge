...
-- calculate topping adjustments from extras and exclusions (eg cheese -4, +1 = -3)
-- apply this adjustment to totals for standard toppings 
SELECT
	topping_name,
    SUM(CASE
...
-- check number of delivered pizzas
SELECT COUNT(order_item_id) AS num_delivered_pizzas
FROM order_items
JOIN orders
